...
def send_notification(operator_id,params):
    ...
    get_email_list = "SELECT Email_id from DONOR_EMAIL as dp join DONOR \
                        AS dn on (dp.Donor_id = dn.Donor_id) WHERE \
                        (Notification_Subscription = '%s' AND Notification_Type='%s'  \
                        AND Blood_Group='O+' AND dn.Donor_id) in \
                        (SELECT Donor_id FROM AFFILIATED WHERE Operator_id='%s')"  
